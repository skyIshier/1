...
用法：
  1. 交互模式：python mesh_converter.py
  2. 命令行：   python mesh_converter.py 文件1.mesh -o 输出目录 --mode [hybrid|old|sky]
  3. 目录模式： python mesh_converter.py --dir 目录路径 --mode [hybrid|old|sky]
"""
...
def main():
    ...
    # ========== 新增：目录模式 ==========
    parser.add_argument('--dir', help='指定目录，自动处理该目录下所有 .mesh 文件（忽略 files 参数）')
    ...
    if args.dir:
        ...
        if not mesh_files:
            print(f"目录 {args.dir} 中没有找到 .mesh 文件")
            return
        out_dir = os.path.join(args.dir, "output")
        os.makedirs(out_dir, exist_ok=True)
        mode = args.mode
        # 直接进入批量处理循环（复用后面的代码）
        # 注意：这里需要手动设置 out_dir，并跳过交互部分
        # 我们将 mesh_files 和 out_dir 准备好后，直接进入转换循环
        # 为了复用，我们可以在下面构造一个类似命令行参数列表的情况，但简单起见直接写循环
        print(f"找到 {len(mesh_files)} 个 .mesh 文件，输出到 {out_dir}")
        # 开始转换（复制了后面的循环代码，但确保变量名一致）
        ...
        return  # 目录模式处理完毕，退出 main

    # ========== 以下是原有逻辑（命令行文件列表或无参数交互）==========
    # 确定输入文件列表
    if args.files:
